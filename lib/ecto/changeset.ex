  ...
  defp convert_params(params) do
    Enum.reduce params, %{}, fn
      {key, value}, acc when is_atom(key) ->
        Dict.put(acc, Atom.to_string(key), value)
      {key, value}, acc when is_binary(key) ->
        if Dict.has_key?(acc, key) do
          raise ArgumentError, "expected params to be a map with unique atom and string" <>
                               "keys, got a map with a duplicate key: #{key}"
        end
